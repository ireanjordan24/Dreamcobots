...
class DealFinderBot:
    ...
    def evaluate_deal(self, item: dict) -> dict:
        ...
        if self.tier in (Tier.PRO, Tier.ENTERPRISE):
            ...
            result["profit_calculator"] = {
                ...
                "fees_estimate": round(
                    (item["market_value"] - item["buy_price"]) * 0.15, 2
                ),
    ...
    def get_best_deals(self, limit: int = 5) -> list:
        ...
        return sorted(annotated, key=lambda x: x["estimated_profit"], reverse=True)[
            :limit
        ]
