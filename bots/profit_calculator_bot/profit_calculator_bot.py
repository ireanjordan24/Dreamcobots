...
class ProfitCalculatorBot:
    ...
    def calculate_break_even(
        self, fixed_costs: float, variable_cost_per_unit: float, selling_price: float
    ) -> dict:
        ...
        contribution_margin_ratio = (
            (contribution_margin / selling_price * 100) if selling_price != 0 else 0.0
        )
        return {
    ...
    def build_pl_statement(
        self, revenue: float, cogs: float, operating_expenses: float
    ) -> dict:
        ...
        operating_margin_pct = (
            (operating_income / revenue * 100) if revenue != 0 else 0.0
        )
    ...
    def optimize_pricing(
        self, cost: float, target_margin: float, competitor_price: float = None
    ) -> dict:
        recommended_price = cost / (1 - target_margin) if target_margin < 1 else cost
        actual_margin = (
            ((recommended_price - cost) / recommended_price * 100)
    ...
    def run_what_if(self, scenario_dict: dict) -> dict:
        ...
        optimistic = _calc(
            base_revenue * (1 + revenue_change + 0.1),
            base_cogs * (1 + cogs_change - 0.05),
        )
        pessimistic = _calc(
            base_revenue * (1 + revenue_change - 0.15),
            base_cogs * (1 + cogs_change + 0.10),
        )
