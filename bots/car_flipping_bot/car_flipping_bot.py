...
class CarFlippingBot:
    ...
    def search_cars(self, make: str, budget: float) -> list:
        ...
        if (
            self.tier == Tier.FREE
            and self._current_make is not None
            and make.lower() != self._current_make.lower()
        ):
    ...
    def _annotate_car(self, car: dict) -> dict:
        ...
        condition_score = {"Very Good": 90, "Good": 70, "Fair": 50, "Poor": 25}.get(
            car["condition"], 60
        )
        return {
            **car,
            ...
            "profit_margin_pct": round(
                (car["market_value"] - car["buy_price"]) / car["market_value"] * 100, 1
            ),
        }
    ...
    def evaluate_car(self, car: dict) -> dict:
        ...
        if self.tier in (Tier.PRO, Tier.ENTERPRISE):
            ...
            result["repair_breakdown"] = {
                "mechanical": round(car.get("repair_cost", 0) * 0.6, 2),
                "cosmetic": round(car.get("repair_cost", 0) * 0.4, 2),
            }
    ...
    def estimate_flip_profit(self, car: dict) -> float:
        ...
        return (
            car["market_value"] - car["buy_price"] - car.get("repair_cost", 0)
        ) * 0.92
    ...
    def get_best_opportunities(self, limit: int = 5) -> list:
        ...
        return sorted(annotated, key=lambda x: x["flip_profit_est"], reverse=True)[
            :limit
        ]
...
def _cfb_create_checkout_session(
    ...
    result = self._stripe.create_checkout_session(
        plan=f"CarFlipping {upgrade_tier.value.title()}", amount_cents=price_cents
    )
    if customer_email:
