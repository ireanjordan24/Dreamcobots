...
class AffiliateBot:
    ...
    def recommend_products(self, niche: str) -> list:
        ...
        if (
            ...
            and len(self._tracked_niches) >= niche_limit
            and niche not in self._tracked_niches
        ):
        ...
        return [
            {
                **p,
                "estimated_monthly_earnings": round(
                    p["avg_price"] * p["commission_rate"] * 30 * multiplier, 2
                ),
    ...
    def estimate_daily_income(self, niche: str) -> dict:
        ...
        conversion_rate = (
            ...
            else (0.035 if self.tier == Tier.PRO else 0.05)
        )
        ...
        for p in products[:5]:
            ...
            daily_earnings = (
                daily_sales * p["avg_price"] * p["commission_rate"] * multiplier
            )
            total_daily += daily_earnings
            breakdown.append(
                {"product": p["name"], "daily_earnings": round(daily_earnings, 2)}
            )
    ...
    def track_clicks(self, product_id: str) -> dict:
        ...
        return {
            ...
            "conversion_rate": round(
                base_conversions / max(self._clicks[product_id], 1) * 100, 2
            ),
            "tier": self.tier.value,
        }
    ...
    def generate_report(self) -> dict:
        ...
        report = {
            ...
            "total_products_tracked": sum(
                len(self.NICHE_DATA.get(n, [])) for n in self._tracked_niches
            ),
            "total_clicks": total_clicks,
            "estimated_monthly_revenue_usd": estimated_revenue,
            "top_products": sorted(
                self._clicks.items(), key=lambda x: x[1], reverse=True
            )[:5],
            "tier": self.tier.value,
        ...
        if self.tier in (Tier.PRO, Tier.ENTERPRISE):
            report["detailed_breakdown"] = {
                pid: {
                    "clicks": c,
                    "est_revenue": round(c * 0.02 * 25.0 * multiplier, 2),
                }
