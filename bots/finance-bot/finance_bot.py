...
class FinanceBot(BaseBot):
    ...
    def build_budget(self, income: float, expenses_dict: dict) -> dict:
        ...
        expense_breakdown = (
            {
                k: {"amount": v, "percent_of_income": round(v / income * 100, 1)}
        ...
        if savings_rate < 10:
            recommendations.append(
                "⚠️ Savings rate below 10% - aim for 20%+ for financial security"
            )
        if (
            "entertainment" in expenses_dict
            and expenses_dict["entertainment"] / income > 0.10
        ):
            recommendations.append(
                "Entertainment spending is above 10% of income - consider reducing"
            )
        if surplus > 0:
            recommendations.append(
                f"Invest ${surplus * 0.5:.2f} in index funds (50% of surplus)"
            )
            recommendations.append(
                f"Build emergency fund with ${surplus * 0.3:.2f} (30% of surplus)"
            )
        return {
    ...
    def analyze_portfolio(self, holdings_dict: dict) -> dict:
        ...
        allocations = (
            {
                asset: round(value / total_value * 100, 1)
                for asset, value in holdings_dict.items()
            }
            if total_value > 0
            else {}
        )
        ...
        return {
            ...
            "diversification_score": (
                "Good"
                if is_diversified and largest_position_pct < 30
    ...
    def forecast_cash_flow(
        ...
        return {
            ...
            "ending_balance": (
                round(projections[-1]["balance"], 2) if projections else current_balance
            ),
            "break_even_month": (
                next((p["month"] for p in projections if p["balance"] > 0), None)
                if current_balance < 0
                else "Already positive"
            ),
            "projections": projections,
            "trend": (
                "Positive" if monthly_net > 0 else "Negative - expenses exceed income"
            ),
        }

    def calculate_roi(
        self, investment: float, returns: float, time_years: float
    ) -> dict:
        ...
        annualized_roi = (
            ((returns / investment) ** (1 / time_years) - 1) * 100
            if time_years > 0
            else 0
        )
        return {
            ...
            "rating": (
                ...
                else "Good (8-15%)" if annualized_roi > 8 else "Below average (<8%)"
            ),
            "comparison": {
    ...
    def optimize_taxes(self, income: float, deductions_dict: dict) -> dict:
        ...
        return {
            ...
            "effective_tax_rate_percent": (
                round(tax / income * 100, 1) if income > 0 else 0
            ),
    ...
    def track_crypto(self, portfolio_dict: dict) -> dict:
        ...
        for h in holdings:
            h["portfolio_percent"] = (
                round(h["value_usd"] / total_value * 100, 1) if total_value > 0 else 0
            )
        return {
    ...
    def financial_health_score(
        self, assets: float, liabilities: float, income: float, expenses: float
    ) -> dict:
        ...
        return {
            ...
            "rating": (
                ...
                if total_score >= 80
                    ...
                    "Good"
                    if total_score >= 60
                    else "Fair" if total_score >= 40 else "Needs Work"
                )
            ),
            "top_recommendations": [
