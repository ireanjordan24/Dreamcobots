...
def test_unsubscribe_nonexistent_does_not_raise():
    bus = EventBus()
    from BuddyAI.event_bus import EventBusError
    with pytest.raises(EventBusError):
        bus.unsubscribe("ev", lambda d: None)  # raises for nonexistent event type
...
def test_subscriber_exception_does_not_stop_others():
    ...
    # Without an error_handler, exceptions propagate from publish()
