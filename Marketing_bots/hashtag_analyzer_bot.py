"""
HashtagAnalyzerBot — hashtag performance analyzer.
...
class HashtagAnalyzerBotTierError(Exception):
...
class HashtagAnalyzerBot:
    """Tier-aware hashtag performance analyzer."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise HashtagAnalyzerBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of hashtag_analyzer_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value hashtag_analyzer_bot item."""
        self._usage_count += 1
