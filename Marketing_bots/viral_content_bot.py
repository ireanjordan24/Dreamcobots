"""
ViralContentBot — viral content strategy bot.
...
class ViralContentBotTierError(Exception):
...
class ViralContentBot:
    """Tier-aware viral content strategy bot."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise ViralContentBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of viral_content_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value viral_content_bot item."""
        self._usage_count += 1
