"""
VideoMarketingBot — video marketing strategy bot.
...
class VideoMarketingBotTierError(Exception):
...
class VideoMarketingBot:
    """Tier-aware video marketing strategy bot."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise VideoMarketingBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of video_marketing_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value video_marketing_bot item."""
        self._usage_count += 1
