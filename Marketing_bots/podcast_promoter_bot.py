"""
PodcastPromoterBot — podcast promotion bot.
...
class PodcastPromoterBotTierError(Exception):
...
class PodcastPromoterBot:
    """Tier-aware podcast promotion bot."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise PodcastPromoterBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of podcast_promoter_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value podcast_promoter_bot item."""
        self._usage_count += 1
