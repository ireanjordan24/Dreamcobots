"""
AnalyticsReporterBot — marketing analytics reporter.
...
class AnalyticsReporterBotTierError(Exception):
...
class AnalyticsReporterBot:
    """Tier-aware marketing analytics reporter."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise AnalyticsReporterBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of analytics_reporter_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value analytics_reporter_bot item."""
        self._usage_count += 1
