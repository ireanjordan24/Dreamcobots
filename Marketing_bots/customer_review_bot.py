"""
CustomerReviewBot — customer review solicitor.
...
class CustomerReviewBotTierError(Exception):
...
class CustomerReviewBot:
    """Tier-aware customer review solicitor."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise CustomerReviewBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of customer_review_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value customer_review_bot item."""
        self._usage_count += 1
