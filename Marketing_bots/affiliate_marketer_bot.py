"""
AffiliateMarketerBot — affiliate marketing manager.
...
class AffiliateMarketerBotTierError(Exception):
...
class AffiliateMarketerBot:
    """Tier-aware affiliate marketing manager."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise AffiliateMarketerBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of affiliate_marketer_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value affiliate_marketer_bot item."""
        self._usage_count += 1
