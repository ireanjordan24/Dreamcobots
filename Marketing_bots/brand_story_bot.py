"""
BrandStoryBot — brand story and positioning bot.
...
class BrandStoryBotTierError(Exception):
...
class BrandStoryBot:
    """Tier-aware brand story and positioning bot."""
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of brand_story_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value brand_story_bot item."""
        self._usage_count += 1
