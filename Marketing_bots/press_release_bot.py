"""
PressReleaseBot — automated press release writer.
...
class PressReleaseBotTierError(Exception):
...
class PressReleaseBot:
    """Tier-aware automated press release writer."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise PressReleaseBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of press_release_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value press_release_bot item."""
        self._usage_count += 1
