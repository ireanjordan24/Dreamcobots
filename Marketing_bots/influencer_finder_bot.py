"""
InfluencerFinderBot — influencer discovery and outreach bot.
...
class InfluencerFinderBotTierError(Exception):
...
class InfluencerFinderBot:
    """Tier-aware influencer discovery and outreach bot."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise InfluencerFinderBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of influencer_finder_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value influencer_finder_bot item."""
        self._usage_count += 1
