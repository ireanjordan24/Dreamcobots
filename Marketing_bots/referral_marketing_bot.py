"""
ReferralMarketingBot — referral marketing program bot.
...
class ReferralMarketingBotTierError(Exception):
...
class ReferralMarketingBot:
    """Tier-aware referral marketing program bot."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise ReferralMarketingBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of referral_marketing_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value referral_marketing_bot item."""
        self._usage_count += 1
