"""
EventPromoterBot — event promotion and ticketing bot.
...
class EventPromoterBotTierError(Exception):
...
class EventPromoterBot:
    """Tier-aware event promotion and ticketing bot."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise EventPromoterBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of event_promoter_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value event_promoter_bot item."""
        self._usage_count += 1
