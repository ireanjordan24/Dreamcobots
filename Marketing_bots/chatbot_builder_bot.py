"""
ChatbotBuilderBot — marketing chatbot builder.
...
class ChatbotBuilderBotTierError(Exception):
...
class ChatbotBuilderBot:
    """Tier-aware marketing chatbot builder."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise ChatbotBuilderBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of chatbot_builder_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value chatbot_builder_bot item."""
        self._usage_count += 1
