"""
RevisionManagerBot — revision request manager.
...
class RevisionManagerBotTierError(Exception):
...
class RevisionManagerBot:
    """Tier-aware revision request manager."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise RevisionManagerBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of revision_manager_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value revision_manager_bot item."""
        self._usage_count += 1
