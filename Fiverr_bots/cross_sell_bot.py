"""
CrossSellBot — cross-selling opportunity finder.
...
class CrossSellBotTierError(Exception):
...
class CrossSellBot:
    """Tier-aware cross-selling opportunity finder."""
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of cross_sell_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value cross_sell_bot item."""
        self._usage_count += 1
