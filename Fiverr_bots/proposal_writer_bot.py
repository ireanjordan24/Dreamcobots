"""
ProposalWriterBot — automated proposal writer.
...
class ProposalWriterBotTierError(Exception):
...
class ProposalWriterBot:
    """Tier-aware automated proposal writer."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise ProposalWriterBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of proposal_writer_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value proposal_writer_bot item."""
        self._usage_count += 1
