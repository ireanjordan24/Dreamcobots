...
def _fiverrservicelisting_bot_new_init(self, tier="FREE"):
...
def _fiverrservicelisting_bot_analyze(self):
    ...
    return {"bot": "FiverrServiceListingBot", "tier": (self.tier.value if hasattr(self.tier, "value") else self.tier.lower()), "count": len(EXAMPLES)}


def _fiverrservicelisting_bot_export_report(self):
    self._enforce_tier("enterprise")
    return {"bot": "FiverrServiceListingBot", "tier": (self.tier.value if hasattr(self.tier, "value") else self.tier.lower()), "total_items": len(EXAMPLES), "items": EXAMPLES}
