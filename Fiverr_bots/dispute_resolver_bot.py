"""
DisputeResolverBot — order dispute resolution assistant.
...
class DisputeResolverBotTierError(Exception):
...
class DisputeResolverBot:
    """Tier-aware order dispute resolution assistant."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise DisputeResolverBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of dispute_resolver_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value dispute_resolver_bot item."""
        self._usage_count += 1
