"""
GigImageOptimizerBot — gig thumbnail optimizer.
...
class GigImageOptimizerBotTierError(Exception):
...
class GigImageOptimizerBot:
    """Tier-aware gig thumbnail optimizer."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise GigImageOptimizerBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of gig_image_optimizer_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value gig_image_optimizer_bot item."""
        self._usage_count += 1
