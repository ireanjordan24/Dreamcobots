"""
ClientRetentionBot — client retention and follow-up bot.
...
class ClientRetentionBotTierError(Exception):
...
class ClientRetentionBot:
    """Tier-aware client retention and follow-up bot."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise ClientRetentionBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of client_retention_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value client_retention_bot item."""
        self._usage_count += 1
