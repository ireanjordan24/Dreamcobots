"""
InboxAutomationBot — inbox message automation bot.
...
class InboxAutomationBotTierError(Exception):
...
class InboxAutomationBot:
    """Tier-aware inbox message automation bot."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise InboxAutomationBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of inbox_automation_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value inbox_automation_bot item."""
        self._usage_count += 1
