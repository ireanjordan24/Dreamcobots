"""
OrderCompletionBot — order completion rate optimizer.
...
class OrderCompletionBotTierError(Exception):
...
class OrderCompletionBot:
    """Tier-aware order completion rate optimizer."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise OrderCompletionBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of order_completion_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value order_completion_bot item."""
        self._usage_count += 1
