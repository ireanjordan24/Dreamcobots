"""
BuyerPersonaBot — ideal buyer persona analyzer.
...
class BuyerPersonaBotTierError(Exception):
...
class BuyerPersonaBot:
    """Tier-aware ideal buyer persona analyzer."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise BuyerPersonaBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of buyer_persona_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value buyer_persona_bot item."""
        self._usage_count += 1
