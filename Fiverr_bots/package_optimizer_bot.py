"""
PackageOptimizerBot — gig package tier optimizer.
...
class PackageOptimizerBotTierError(Exception):
...
class PackageOptimizerBot:
    """Tier-aware gig package tier optimizer."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise PackageOptimizerBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of package_optimizer_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value package_optimizer_bot item."""
        self._usage_count += 1
