"""
SeasonalPricingBot — seasonal pricing strategy bot.
...
class SeasonalPricingBotTierError(Exception):
...
class SeasonalPricingBot:
    """Tier-aware seasonal pricing strategy bot."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise SeasonalPricingBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of seasonal_pricing_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value seasonal_pricing_bot item."""
        self._usage_count += 1
