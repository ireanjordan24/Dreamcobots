"""
DeadlineTrackerBot — project deadline tracker.
...
class DeadlineTrackerBotTierError(Exception):
...
class DeadlineTrackerBot:
    """Tier-aware project deadline tracker."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise DeadlineTrackerBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of deadline_tracker_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value deadline_tracker_bot item."""
        self._usage_count += 1
