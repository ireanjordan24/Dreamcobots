"""
CompetitorSpyBot — competitor gig spy and analyzer.
...
class CompetitorSpyBotTierError(Exception):
...
class CompetitorSpyBot:
    """Tier-aware competitor gig spy and analyzer."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise CompetitorSpyBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of competitor_spy_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value competitor_spy_bot item."""
        self._usage_count += 1
