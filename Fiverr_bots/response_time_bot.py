"""
ResponseTimeBot — response time optimizer.
...
class ResponseTimeBotTierError(Exception):
...
class ResponseTimeBot:
    """Tier-aware response time optimizer."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise ResponseTimeBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of response_time_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value response_time_bot item."""
        self._usage_count += 1
