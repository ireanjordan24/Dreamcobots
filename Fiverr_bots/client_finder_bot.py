"""
ClientFinderBot — ideal client finder.
...
class ClientFinderBotTierError(Exception):
...
class ClientFinderBot:
    """Tier-aware ideal client finder."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise ClientFinderBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of client_finder_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value client_finder_bot item."""
        self._usage_count += 1
