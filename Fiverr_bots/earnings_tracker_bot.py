"""
EarningsTrackerBot — freelance earnings tracker.
...
class EarningsTrackerBotTierError(Exception):
...
class EarningsTrackerBot:
    """Tier-aware freelance earnings tracker."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise EarningsTrackerBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of earnings_tracker_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value earnings_tracker_bot item."""
        self._usage_count += 1
