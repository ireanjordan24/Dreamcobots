"""
AccessibilityBot — accessibility compliance checker.
...
class AccessibilityBotTierError(Exception):
...
class AccessibilityBot:
    """Tier-aware accessibility compliance checker."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise AccessibilityBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of accessibility_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value accessibility_bot item."""
        self._usage_count += 1
