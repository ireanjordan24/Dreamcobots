"""
PerformanceMonitorBot — app performance monitor.
...
class PerformanceMonitorBotTierError(Exception):
...
class PerformanceMonitorBot:
    """Tier-aware app performance monitor."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise PerformanceMonitorBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of performance_monitor_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value performance_monitor_bot item."""
        self._usage_count += 1
