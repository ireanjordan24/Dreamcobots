"""
PersonalizationBot — user experience personalization bot.
...
class PersonalizationBotTierError(Exception):
...
class PersonalizationBot:
    """Tier-aware user experience personalization bot."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise PersonalizationBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of personalization_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value personalization_bot item."""
        self._usage_count += 1
