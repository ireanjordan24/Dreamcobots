"""
AppStoreOptimizerBot — app store listing optimizer.
...
class AppStoreOptimizerBotTierError(Exception):
...
class AppStoreOptimizerBot:
    """Tier-aware app store listing optimizer."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise AppStoreOptimizerBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of app_store_optimizer_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value app_store_optimizer_bot item."""
        self._usage_count += 1
