"""
MonetizationBot — app monetization strategy advisor.
...
class MonetizationBotTierError(Exception):
...
class MonetizationBot:
    """Tier-aware app monetization strategy advisor."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise MonetizationBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of monetization_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value monetization_bot item."""
        self._usage_count += 1
