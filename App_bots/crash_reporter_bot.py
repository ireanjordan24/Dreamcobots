"""
CrashReporterBot — crash report analyzer and triage bot.
...
class CrashReporterBotTierError(Exception):
...
class CrashReporterBot:
    """Tier-aware crash report analyzer and triage bot."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise CrashReporterBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of crash_reporter_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value crash_reporter_bot item."""
        self._usage_count += 1
