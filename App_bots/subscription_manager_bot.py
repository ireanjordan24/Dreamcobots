"""
SubscriptionManagerBot — subscription lifecycle manager.
...
class SubscriptionManagerBotTierError(Exception):
...
class SubscriptionManagerBot:
    """Tier-aware subscription lifecycle manager."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise SubscriptionManagerBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of subscription_manager_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value subscription_manager_bot item."""
        self._usage_count += 1
