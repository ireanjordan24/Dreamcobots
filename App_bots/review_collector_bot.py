"""
ReviewCollectorBot — app-store review collector and analyzer.
...
class ReviewCollectorBotTierError(Exception):
...
class ReviewCollectorBot:
    """Tier-aware app-store review collector and analyzer."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise ReviewCollectorBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of review_collector_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value review_collector_bot item."""
        self._usage_count += 1
