"""
UserFeedbackBot — in-app user feedback collector.
...
class UserFeedbackBotTierError(Exception):
...
class UserFeedbackBot:
    """Tier-aware in-app user feedback collector."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise UserFeedbackBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of user_feedback_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value user_feedback_bot item."""
        self._usage_count += 1
