"""
InAppPurchaseBot — in-app purchase optimizer.
...
class InAppPurchaseBotTierError(Exception):
...
class InAppPurchaseBot:
    """Tier-aware in-app purchase optimizer."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise InAppPurchaseBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of in_app_purchase_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value in_app_purchase_bot item."""
        self._usage_count += 1
