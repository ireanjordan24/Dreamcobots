"""
ChurnPredictorBot — user churn predictor.
...
class ChurnPredictorBotTierError(Exception):
...
class ChurnPredictorBot:
    """Tier-aware user churn predictor."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise ChurnPredictorBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of churn_predictor_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value churn_predictor_bot item."""
        self._usage_count += 1
