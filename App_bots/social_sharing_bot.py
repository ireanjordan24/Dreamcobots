"""
SocialSharingBot — social sharing and virality tracker.
...
class SocialSharingBotTierError(Exception):
...
class SocialSharingBot:
    """Tier-aware social sharing and virality tracker."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise SocialSharingBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of social_sharing_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value social_sharing_bot item."""
        self._usage_count += 1
