"""
OnboardingFunnelBot — onboarding funnel optimizer.
...
class OnboardingFunnelBotTierError(Exception):
...
class OnboardingFunnelBot:
    """Tier-aware onboarding funnel optimizer."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise OnboardingFunnelBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of onboarding_funnel_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value onboarding_funnel_bot item."""
        self._usage_count += 1
