"""
PushNotificationBot — personalized push notification sender.
...
class PushNotificationBotTierError(Exception):
...
class PushNotificationBot:
    """Tier-aware personalized push notification sender."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise PushNotificationBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of push_notification_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value push_notification_bot item."""
        self._usage_count += 1
