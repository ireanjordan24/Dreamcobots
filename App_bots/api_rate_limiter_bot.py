"""
APIRateLimiterBot — API rate limit manager.
...
class APIRateLimiterBotTierError(Exception):
...
class APIRateLimiterBot:
    """Tier-aware API rate limit manager."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise APIRateLimiterBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of api_rate_limiter_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value api_rate_limiter_bot item."""
        self._usage_count += 1
