...
def _usersupport_bot_analyze(self):
    ...
    return {"bot": "UserSupportBot", "tier": self.tier.lower(), "count": len(EXAMPLES)}


def _usersupport_bot_export_report(self):
    self._enforce_tier("enterprise")
    return {"bot": "UserSupportBot", "tier": self.tier.lower(), "total_items": len(EXAMPLES), "items": EXAMPLES}
