"""
GamificationBot — gamification and rewards engine.
...
class GamificationBotTierError(Exception):
...
class GamificationBot:
    """Tier-aware gamification and rewards engine."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise GamificationBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of gamification_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value gamification_bot item."""
        self._usage_count += 1
