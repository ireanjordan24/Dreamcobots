...
class HedgeFundStrategy:
    ...
    def generate_long_short(self, universe: list) -> dict:
        ...
        shorts = [
            s[0]["ticker"]
            for s in sorted(scores, key=lambda x: x[1])[: len(universe) // 2]
        ]
        return {
            "long": longs,
            "short": shorts,
        ...
        return [
            {
                "event": e.get("type", ""),
                "ticker": e.get("ticker", ""),
                "alpha_est": round((hash(e.get("ticker", "")) % 20 - 10) / 100, 4),
            }
            for e in events
        ]
    ...
    def analyze_factors(self, portfolio: dict) -> dict:
        ...
        return {
            "value_tilt": round(
                sum(h.get("pb_ratio", 1) for h in holdings) / max(len(holdings), 1), 3
            ),
            "momentum_tilt": round(
                sum(h.get("momentum", 0) for h in holdings) / max(len(holdings), 1), 3
            ),
            "disclaimer": DISCLAIMER,
        }
