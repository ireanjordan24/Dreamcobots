"""
InvestorMatchmakerBot — investor-to-deal matchmaker.
...
class InvestorMatchmakerBotTierError(Exception):
...
class InvestorMatchmakerBot:
    """Tier-aware investor-to-deal matchmaker."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise InvestorMatchmakerBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of investor_matchmaker_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value investor_matchmaker_bot item."""
        self._usage_count += 1
