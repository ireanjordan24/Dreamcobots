"""
ComparableSalesBot — comparable sales (comps) analyzer.
...
class ComparableSalesBotTierError(Exception):
...
class ComparableSalesBot:
    """Tier-aware comparable sales (comps) analyzer."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise ComparableSalesBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of comparable_sales_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value comparable_sales_bot item."""
        self._usage_count += 1
