"""
RenovationCostBot — renovation cost estimator.
...
class RenovationCostBotTierError(Exception):
...
class RenovationCostBot:
    """Tier-aware renovation cost estimator."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise RenovationCostBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of renovation_cost_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value renovation_cost_bot item."""
        self._usage_count += 1
