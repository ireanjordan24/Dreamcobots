"""
TenantScreeningBot — tenant background and credit screener.
...
class TenantScreeningBotTierError(Exception):
...
class TenantScreeningBot:
    """Tier-aware tenant background and credit screener."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise TenantScreeningBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of tenant_screening_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value tenant_screening_bot item."""
        self._usage_count += 1
