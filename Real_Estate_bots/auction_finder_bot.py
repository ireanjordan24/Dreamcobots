"""
AuctionFinderBot — property auction finder.
...
class AuctionFinderBotTierError(Exception):
...
class AuctionFinderBot:
    """Tier-aware property auction finder."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise AuctionFinderBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of auction_finder_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value auction_finder_bot item."""
        self._usage_count += 1
