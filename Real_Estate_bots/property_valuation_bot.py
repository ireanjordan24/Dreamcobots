"""
PropertyValuationBot — AI-driven property valuation.
...
class PropertyValuationBotTierError(Exception):
...
class PropertyValuationBot:
    """Tier-aware AI-driven property valuation."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise PropertyValuationBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of property_valuation_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value property_valuation_bot item."""
        self._usage_count += 1
