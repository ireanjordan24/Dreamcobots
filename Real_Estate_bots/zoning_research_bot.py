"""
ZoningResearchBot — zoning and land-use researcher.
...
class ZoningResearchBotTierError(Exception):
...
class ZoningResearchBot:
    """Tier-aware zoning and land-use researcher."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise ZoningResearchBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of zoning_research_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value zoning_research_bot item."""
        self._usage_count += 1
