"""
NeighborhoodScorerBot — neighborhood quality scorer.
...
class NeighborhoodScorerBotTierError(Exception):
...
class NeighborhoodScorerBot:
    """Tier-aware neighborhood quality scorer."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise NeighborhoodScorerBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of neighborhood_scorer_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value neighborhood_scorer_bot item."""
        self._usage_count += 1
