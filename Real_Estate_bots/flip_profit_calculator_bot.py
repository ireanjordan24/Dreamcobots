"""
FlipProfitCalculatorBot — fix-and-flip profit calculator.
...
class FlipProfitCalculatorBotTierError(Exception):
...
class FlipProfitCalculatorBot:
    """Tier-aware fix-and-flip profit calculator."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise FlipProfitCalculatorBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of flip_profit_calculator_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value flip_profit_calculator_bot item."""
        self._usage_count += 1
