"""
PropertyAlertBot — property price drop alert system.
...
class PropertyAlertBotTierError(Exception):
...
class PropertyAlertBot:
    """Tier-aware property price drop alert system."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise PropertyAlertBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of property_alert_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value property_alert_bot item."""
        self._usage_count += 1
