"""
LeaseGeneratorBot — automated lease document generator.
...
class LeaseGeneratorBotTierError(Exception):
...
class LeaseGeneratorBot:
    """Tier-aware automated lease document generator."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise LeaseGeneratorBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of lease_generator_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value lease_generator_bot item."""
        self._usage_count += 1
