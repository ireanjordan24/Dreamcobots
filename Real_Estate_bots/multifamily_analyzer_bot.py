"""
MultifamilyAnalyzerBot — multifamily property analyzer.
...
class MultifamilyAnalyzerBotTierError(Exception):
...
class MultifamilyAnalyzerBot:
    """Tier-aware multifamily property analyzer."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise MultifamilyAnalyzerBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of multifamily_analyzer_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value multifamily_analyzer_bot item."""
        self._usage_count += 1
