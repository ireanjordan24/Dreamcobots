"""
PropertyManagementBot — property management assistant.
...
class PropertyManagementBotTierError(Exception):
...
class PropertyManagementBot:
    """Tier-aware property management assistant."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise PropertyManagementBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of property_management_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value property_management_bot item."""
        self._usage_count += 1
