"""
RentalIncomeBot — rental income and yield analyzer.
...
class RentalIncomeBotTierError(Exception):
...
class RentalIncomeBot:
    """Tier-aware rental income and yield analyzer."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise RentalIncomeBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of rental_income_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value rental_income_bot item."""
        self._usage_count += 1
