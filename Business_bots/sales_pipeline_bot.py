"""
SalesPipelineBot — sales pipeline manager.
...
class SalesPipelineBotTierError(Exception):
...
class SalesPipelineBot:
    """Tier-aware sales pipeline manager."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise SalesPipelineBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of sales_pipeline_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value sales_pipeline_bot item."""
        self._usage_count += 1
