"""
InsuranceAdvisorBot — business insurance advisor.
...
class InsuranceAdvisorBotTierError(Exception):
...
class InsuranceAdvisorBot:
    """Tier-aware business insurance advisor."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise InsuranceAdvisorBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of insurance_advisor_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value insurance_advisor_bot item."""
        self._usage_count += 1
