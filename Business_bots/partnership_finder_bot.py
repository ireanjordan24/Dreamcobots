"""
PartnershipFinderBot — business partnership finder.
...
class PartnershipFinderBotTierError(Exception):
...
class PartnershipFinderBot:
    """Tier-aware business partnership finder."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise PartnershipFinderBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of partnership_finder_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value partnership_finder_bot item."""
        self._usage_count += 1
