"""
BrandMonitorBot — brand mention monitor.
...
class BrandMonitorBotTierError(Exception):
...
class BrandMonitorBot:
    """Tier-aware brand mention monitor."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise BrandMonitorBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of brand_monitor_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value brand_monitor_bot item."""
        self._usage_count += 1
