"""
PricingStrategyBot — pricing strategy advisor.
...
class PricingStrategyBotTierError(Exception):
...
class PricingStrategyBot:
    """Tier-aware pricing strategy advisor."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise PricingStrategyBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of pricing_strategy_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value pricing_strategy_bot item."""
        self._usage_count += 1
