"""
InventoryManagerBot — inventory management bot.
...
class InventoryManagerBotTierError(Exception):
...
class InventoryManagerBot:
    """Tier-aware inventory management bot."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise InventoryManagerBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of inventory_manager_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value inventory_manager_bot item."""
        self._usage_count += 1
