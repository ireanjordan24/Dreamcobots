"""
MarketResearchBot — market research aggregator.
...
class MarketResearchBotTierError(Exception):
...
class MarketResearchBot:
    """Tier-aware market research aggregator."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise MarketResearchBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of market_research_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value market_research_bot item."""
        self._usage_count += 1
