...
def _invoicing_bot_analyze(self):
    ...
    return {"bot": "InvoicingBot", "tier": (self.tier.value if hasattr(self.tier, "value") else self.tier.lower()), "count": len(EXAMPLES)}


def _invoicing_bot_export_report(self):
    self._enforce_tier("enterprise")
    return {"bot": "InvoicingBot", "tier": (self.tier.value if hasattr(self.tier, "value") else self.tier.lower()), "total_items": len(EXAMPLES), "items": EXAMPLES}
