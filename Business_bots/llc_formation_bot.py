"""
LLCFormationBot — LLC formation guide bot.
...
class LLCFormationBotTierError(Exception):
...
class LLCFormationBot:
    """Tier-aware LLC formation guide bot."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise LLCFormationBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of llc_formation_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value llc_formation_bot item."""
        self._usage_count += 1
