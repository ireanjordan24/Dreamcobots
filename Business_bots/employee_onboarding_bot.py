"""
EmployeeOnboardingBot — employee onboarding assistant.
...
class EmployeeOnboardingBotTierError(Exception):
...
class EmployeeOnboardingBot:
    """Tier-aware employee onboarding assistant."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise EmployeeOnboardingBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of employee_onboarding_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value employee_onboarding_bot item."""
        self._usage_count += 1
