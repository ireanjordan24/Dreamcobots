"""
BusinessPlanBot — AI business plan generator.
...
class BusinessPlanBotTierError(Exception):
...
class BusinessPlanBot:
    """Tier-aware AI business plan generator."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise BusinessPlanBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of business_plan_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value business_plan_bot item."""
        self._usage_count += 1
