"""
MeetingSchedulerBot — smart meeting scheduler.
...
class MeetingSchedulerBotTierError(Exception):
...
class MeetingSchedulerBot:
    """Tier-aware smart meeting scheduler."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise MeetingSchedulerBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of meeting_scheduler_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value meeting_scheduler_bot item."""
        self._usage_count += 1
