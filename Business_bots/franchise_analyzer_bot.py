"""
FranchiseAnalyzerBot — franchise opportunity analyzer.
...
class FranchiseAnalyzerBotTierError(Exception):
...
class FranchiseAnalyzerBot:
    """Tier-aware franchise opportunity analyzer."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise FranchiseAnalyzerBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of franchise_analyzer_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value franchise_analyzer_bot item."""
        self._usage_count += 1
