"""
FinancialForecasterBot — financial forecast modeler.
...
class FinancialForecasterBotTierError(Exception):
...
class FinancialForecasterBot:
    """Tier-aware financial forecast modeler."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise FinancialForecasterBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of financial_forecaster_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value financial_forecaster_bot item."""
        self._usage_count += 1
