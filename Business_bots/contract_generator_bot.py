"""
ContractGeneratorBot — contract and NDA generator.
...
class ContractGeneratorBotTierError(Exception):
...
class ContractGeneratorBot:
    """Tier-aware contract and NDA generator."""
    ...
    def _enforce_tier(self, required_value: str) -> None:
        ...
        if order.index(self.tier.value) < order.index(required_value):
            raise ContractGeneratorBotTierError(
    ...
    def list_items(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Return a list of contract_generator_bot items up to the tier limit."""
        self._usage_count += 1
    ...
    def get_top_item(self) -> Dict[str, Any]:
        """Return the highest-value contract_generator_bot item."""
        self._usage_count += 1
