...
async function loadLeads(industry = "") {
  ...
  showError("");
  try {
    const url = industry ? `${API_BASE}/leads?industry=${encodeURIComponent(industry)}` : `${API_BASE}/leads`;
...
function renderLeads(leads) {
  ...
  if (!leads || leads.length === 0) {
    tbody.innerHTML = `<tr><td colspan="7" class="empty-state">No leads found. Click <strong>Scrape Leads</strong> to get started.</td></tr>`;
    return;
  }
...
function filterLeads() {
  ...
  if (query) {
    filtered = filtered.filter((l) =>
      [l.name, l.company, l.email, l.industry, l.location]
...
function renderBots(bots) {
  const grid = $("botsGrid");
  if (!bots.length) {
    grid.innerHTML = `<div class="empty-state">No bots yet. Click <strong>Create Bot</strong> to get started.</div>`;
    return;
  }
...
function handlePlanClick(plan) {
  if (plan === "buy-leads") {
    ...
    if (!qty) return;
    showToast(`💳 Redirecting to checkout for ${qty} leads… (Stripe integration coming soon)`, "info");
  } else {
