...
class BuddyBot:
    ...
    def register_bot(self, name: str, bot_instance: object) -> None:
        ...
        # Publish under both naming conventions for backwards compatibility
    ...
    def unregister_bot(self, name: str) -> None:
        ...
        # Publish under both naming conventions for backwards compatibility
        self._event_bus.publish("bot_unregistered", {"name": name})
    ...
    def route_message(self, bot_name: str, message: str, **kwargs) -> dict:
        ...
        Tries ``chat()`` first (for chat-oriented bots), then falls back to
        ``process()``.  Raises ``BuddyBotError`` if neither method is available.
        ...
        **kwargs
        ...
        elif callable(getattr(bot, "process", None)):
    ...
    def broadcast(self, message: str) -> dict:
        ...
        for name, bot in self._registered_bots.items():
            if callable(getattr(bot, "chat", None)):
                results[name] = bot.chat(message)
        self._event_bus.publish("broadcast_sent", {"message": message})
        return results
