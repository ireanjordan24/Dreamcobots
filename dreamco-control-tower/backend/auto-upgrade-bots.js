...
function commitAndPush(repoPath, message) {
  try {
    ...
    if (!status.trim()) return { committed: false, pushed: false };
...
export async function selfHeal(botConfig, token) {
  ...
  try {
    ...
    if (!failedRun) return { skipped: true, reason: "No failed runs found" };
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  if (botFlag !== -1) {
    ...
    const bots = loadBots();
    const botConfig = bots.find(
      (b) => b.name.toLowerCase() === botName?.toLowerCase()
    );
    if (!botConfig) {
